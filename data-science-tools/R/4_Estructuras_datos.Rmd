...
Con cierta similaridad a un vector, una **lista** es una colección de objetos, indexable a través del operador **`[[ ]]`** (doble corchete). Soporta desde el mismo tipo de dato a diferentes tipos de datos. La declaración de una lista se realiza desde la función **`list`**.
...
```{r}
df[1] # Para seleccionar la primera columna
```
